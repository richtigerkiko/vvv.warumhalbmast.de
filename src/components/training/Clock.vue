<template>
  <canvas id="timerclock" :width="canvasSize" :height="canvasSize" style="margin: 0px auto; display: block;"></canvas>
</template>
...
export default defineComponent({
  ...
  computed: {
      ctx(): CanvasRenderingContext2D {
          return (document.getElementById('timerclock')! as HTMLCanvasElement).getContext('2d')!
      },
      radius(): number {
  ...
  methods: {
      ...
      drawTimeFinishedIndicator () {
          ...
          this.ctx.beginPath()
          this.ctx.arc(this.radius, this.radius, this.radius -19.5, this.rad(270), this.percentToRad(this.calcRemainingPercentOfTime()), false)
          this.ctx.stroke()
      },
      writeRemainingTimeText () {
          ...
          this.ctx.fillText(Math.floor(this.calcremainingSeconds()).toString(), this.radius - 1, this.radius - 15);
      },
